 ...
 * Requires authentication via optionalApiKeyAuth middleware
 */
router.get("/", apiKeyAuthDashboard, (req: Request, res: Response) => {
	try {
		...
		if (fromStr && isNaN(from!.getTime())) {
			return res
				.status(400)
				.json({ error: "Invalid from date format. Use ISO 8601." });
		}

		if (toStr && isNaN(to!.getTime())) {
			return res
				.status(400)
				.json({ error: "Invalid to date format. Use ISO 8601." });
		}
	...
	(req: Request, res: Response) => {
		try {
			...
			if (!monthStr) {
				return res
					.status(400)
					.json({ error: "month is required (format: MM-YYYY)" });
			}
			...
			if (!month || !year || month < 1 || month > 12) {
				return res
					.status(400)
					.json({ error: "Invalid month format. Use MM-YYYY" });
			}
